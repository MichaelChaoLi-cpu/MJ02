#!/usr/bin/env python3
"""Acquire annual nationwide MCD12Q1 LC_Type1 clips for dynamic cropland masks."""
...
OUTPUT_DIR = Path("data/raw/land_cover/modis_mcd12q1_v061_dynamic")
        ...
        with rasterio.open(url) as source:
            ...
            with rasterio.open(destination, "w", **profile) as target:
                ...
                target.update_tags(
                    ...
                    band="LC_Type1",
                    classification="IGBP",
                    purpose="dynamic annual cropland mask",
                )
            return {
...
def main() -> None:
    args = parse_args()
    if args.start_year < 2001 or args.end_year > 2021 or args.start_year > args.end_year:
        raise ValueError("The selected public MCD12Q1 mosaic release supports 2001-2021")
    ...
    for year in range(args.start_year, args.end_year + 1):
        ...
        print(f"Prepared dynamic MCD12Q1 {year}: {destination.relative_to(root)}")

    pd.DataFrame(rows).to_csv(output_dir / "source_manifest.csv", index=False)
    metadata = {
        "title": "MODIS MCD12Q1 annual land-cover classification for Cambodia",
        ...
        "purpose": "dynamic annual cropland shares for MOD13Q1 NDVI weighting",
    }
